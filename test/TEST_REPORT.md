...
| CSV export returns compact short-date columns | Passed |
...
| `test_csv_export_uses_short_dates` | `GET /complaints/export?start_date=2026-04-01&end_date=2026-04-30` | Confirms exported `created_date` and populated `closed_date` values use compact `DD-MM-YY` formatting for spreadsheet display. |
...
complaint filtering, CSV export date formatting, complaint CRUD operations,
and administrator closure validation. It does not currently cover the frontend
dashboard interactions, Supabase-backed persistence, authorization behavior,
or error cases such as missing complaint IDs.
...
2. Exported date fields are emitted as `DD-MM-YY`, for example `18-04-26`,
   to keep spreadsheet date columns compact.

## Conclusion

The existing API test suite passes in an isolated local test run, including
coverage for compact CSV date exports. The test setup should declare its
runner dependency.
